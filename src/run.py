...
def _check_prev_results(cfg: dict, result_path: str, exp_name: str) -> Tuple[dict, pd.DataFrame]:
    ...
    """
    if not os.path.exists(result_path) or not os.path.exists(f"{result_path}/{exp_name}.csv"):
        ...
        for k, v in cfg[0].items():
            if type(v) == list:
                df_to_check.loc[:, k] = df_to_check[k].apply(lambda x: ast.literal_eval(x) if isinstance(x, str) else x)

        # Remove already executed experiments
        del_count = 0
        for entry in cfg[:]:
            mask = (df_to_check[list(entry.keys())] == pd.Series(entry)).all(axis=1)
            if mask.any():
                cfg.remove(entry)
                del_count += 1
        
        print(f"---{del_count} simulations removed (already executed previously)---")
        return cfg, df
...
def _run_single_experiment(c: dict):
    ...
    return c, top1_perc, top1_perc


def run_experiments(exp: ExpConfig, exp_name: str):
    ...
    # TODO: Determine the baseline accuracy

    res = Parallel(n_jobs = -2, backend='loky', timeout=None)(
        delayed(_run_single_experiment)(c)
        for c in cfgs
    )
    ...
    for cfg, top1, top5 in res:
        metrics = {
            'top1': top1,
            'top5': top5,
            'top1_baseline': 0, # TODO
            'top5_baseline': 0  # TODO
        }
        cfg.update(metrics)
